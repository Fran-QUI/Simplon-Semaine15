class Personne
  ...
  def info
    ...
    if @en_vie
      return "#{@nom} est en vie (#{@points_de_vie} PV)."
  ...
  def subit_attaque(degats_recus)
    ...
    # - Affiche ce qu'il s'est passé
    puts "#{@nom} à perdu #{degats_recus} points_de_vie" 
...
class Joueur < Personne
  ...
  def degats
    # A faire:
    # - Calculer les dégats
    degats = @degats_bonus + rand(15..30)
    # - Affiche ce qu'il s'est passé
...
class Jeu
  ...
  def self.est_fini(joueur, monde)
    ...
    ennemis_en_vie = false
    monde.ennemis.each do |ennemi|
      if ennemi.en_vie
        ennemis_en_vie = true
      end
    end
    if joueur.en_vie and ennemis_en_vie
      return false
    else
      return true
    end
